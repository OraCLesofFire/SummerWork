...
def variableEntry(row, col, c):
    if c == '?':
        return [bddAPI.Bool("({},{})".format(row, col))]
    return []


def variableMatrix(K):
    return FixList(
        [FixList([variableEntry(row, col, c) for (col, c) in enumerate(cols)]) for (row, cols) in enumerate(K)])
...
def tuplevar(s):
    if s[0] == '!':
        return bddAPI.Not(bddAPI.Bool(s[1:]))
    return bddAPI.Bool(s)


def forcedvar(v, row, col):
    if v == '@':
        return True, bddAPI.Not(bddAPI.Bool("({},{})".format(row, col)))
    if v == '!':
        return True, bddAPI.Bool("({},{})".format(row, col))
    return False, None
...
def processFile(path):
    ...
    with open(path) as fp:
        ...
        for line in fp:
            ...
            if cols != len(entries):
                raise Exception(
                    "Rows must contain a uniform number of columns but |row(0)| = {} and |row({})| = {}".format(cols,
                                                                                                                row,
                                                                                                                len(entries)))
            for (col, e) in enumerate(entries):
                if e not in ['!', '@', 'x', '?', '0', '1', '2', '3', '4', '5', '6', '7', '8']:
